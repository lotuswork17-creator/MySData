...
function computeBookCompare(allRecords){
  ...
  function eligible(r, F){
    return bcNz(r,F.oh)&&bcNz(r,F.oa)&&bcNz(r,F.mh)&&bcNz(r,F.ma)&&bcNz(r,F.sh)&&bcNz(r,F.sa)&&bcLineMatch(r);
  }
  ...
  function leanStudy(F){
    ...
    data.forEach(function(r){
      ...
      if(lh==null||lm==null||ls==null) return;
      elig++;
      var pp=(lh-(lm+ls)/2)*100;
  ...
  return {
    total:data.length,
    latestLean: leanStudy(BC_PHASES.latest),
    openingLean: leanStudy(BC_PHASES.opening),
    latestPrice: priceStudy(BC_PHASES.latest),
    openingPrice: priceStudy(BC_PHASES.opening)
  };
}
...
function renderBookCompare(RD){
  ...
  function leanTable(title, sub, study){
    ...
    t+='<div class="rpt-table-wrap"><table class="rpt-table" style="font-size:10px"><thead><tr>'
      +'<th>HKJC home lean vs Macau+SBO consensus</th><th class="num">N</th>'
  ...
  h+=leanTable('① Opening Odds — HKJC lean vs consensus',
    ...
    bc.openingLean);
  h+=leanTable('② Latest Odds — HKJC lean vs consensus',
